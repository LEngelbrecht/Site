<?php
/**
 * @version		$Id: ckforms.php 2012-06-04 sarki$
 * @package		CKForms
 * @subpackage	Content
 * @copyright	Copyright (C) 2008 - 2012 Open Source Matters. All rights reserved.
...
class plgContentCkForms extends JPlugin
{
	...
	function prepareContent( &$article ) {
        ...
        if ($count > 0) {
            foreach ($matches[1] as $i => $textFound) 
			{
				...
				if ($form == null)
				{
				   $html = "<p><strong><span style='color: #660000'>Message d'erreur de CKForms : <br />Formulaire  '" . $textFound . "' introuvable !!!</span></strong></p>";
	 ...
	 *
	 * Method is called by onContentPrepare method
	 *
	...
	function generateCkFormHTML( $form, $fields)
	{
		...
		$document->addScript(JURI::root(true).'/components/com_ckforms/js/calendarv3.js');
		$document->addScript(JURI::root(true).'/components/com_ckforms/js/formcheck.js');
		...
		{
			$html = $html.'<h2 class="ckformtitleplug'.$pageclass_sfx.'">'.$form->title.'</h2>'."\n";
		}
		...
		$html = $html.'},'."\n";
		$html = $html.' alerts : {'."\n";
		$html = $html.'	required:\''.addslashes(JText::_( 'COM_CKFORMS_REQUIRED' )).'\','."\n";
		$html = $html.'	number:\''.addslashes(JText::_( 'COM_CKFORMS_MSG_08' )).'\','."\n";
		$html = $html.'	email:\''.addslashes(JText::_( 'COM_CKFORMS_MSG_09' )).'\','."\n";
		$html = $html.'	url:\''.addslashes(JText::_( 'COM_CKFORMS_MSG_10' )).'\','."\n";
		$html = $html.'	confirm:\''.addslashes(JText::_( 'COM_CKFORMS_MSG_11' )).'\','."\n";
		$html = $html.'	length_str:\''.addslashes(JText::_( 'COM_CKFORMS_MSG_12' )).'\','."\n";
		$html = $html.'	lengthmax:\''.addslashes(JText::_( 'COM_CKFORMS_MSG_13' )).'\','."\n";
		$html = $html.'	lengthmin:\''.addslashes(JText::_( 'COM_CKFORMS_MSG_14' )).'\','."\n";
		$html = $html.'	checkbox:\''.addslashes(JText::_( 'COM_CKFORMS_MSG_15' )).'\','."\n";
		$html = $html.'	radios:\''.addslashes(JText::_( 'COM_CKFORMS_MSG_16' )).'\','."\n";
		$html = $html.'	select:\''.addslashes(JText::_( 'COM_CKFORMS_MSG_17' )).'\''."\n";
		$html = $html.'}'."\n";
		...
		{ 
            ...
            $html = $html.'	{ '."\n";
            $html = $html.'			days: [\''.addslashes(JText::_( 'COM_CKFORMS_DAY_1' )).'\',\''.addslashes(JText::_( 'COM_CKFORMS_DAY_2' )).'\',\''.addslashes(JText::_( 'COM_CKFORMS_DAY_3' )).'\',\''.addslashes(JText::_( 'COM_CKFORMS_DAY_4' )).'\',\''.addslashes(JText::_( 'COM_CKFORMS_DAY_5' )).'\',\''.addslashes(JText::_( 'COM_CKFORMS_DAY_6' )).'\',\''.addslashes(JText::_( 'COM_CKFORMS_DAY_7' )).'\'], '."\n";
            $html = $html.'			months: [\''.addslashes(JText::_( 'COM_CKFORMS_MOIS_1' )).'\',\''.addslashes(JText::_( 'COM_CKFORMS_MOIS_2' )).'\',\''.addslashes(JText::_( 'COM_CKFORMS_MOIS_3' )).'\',\''.addslashes(JText::_( 'COM_CKFORMS_MOIS_4' )).'\',\''.addslashes(JText::_( 'COM_CKFORMS_MOIS_5' )).'\',\''.addslashes(JText::_( 'COM_CKFORMS_MOIS_6' )).'\',\''.addslashes(JText::_( 'COM_CKFORMS_MOIS_7' )).'\',\''.addslashes(JText::_( 'COM_CKFORMS_MOIS_8' )).'\',\''.addslashes(JText::_( 'COM_CKFORMS_MOIS_9' )).'\',\''.addslashes(JText::_( 'COM_CKFORMS_MOIS_10' )).'\',\''.addslashes(JText::_( 'COM_CKFORMS_MOIS_11' )).'\',\''.addslashes(JText::_( 'COM_CKFORMS_MOIS_12' )).'\'],'."\n";
	...
	$html = $html.'<div class="contentpaneopen'.$pageclass_sfx.'" id="ckformcontainer_'.$form->id.'">'."\n";	
	$html = $html.''."\n";	

	if (strcmp ( $form->description , "" ) != 0) {
		$html = $html.'<div id="ckformdescript">'.$form->description.'</div>'."\n";	
	}

	if ($mandatory == true)
	{
		$html = $html.'<div class="ck_mandatory_desc">* '.JText::_( 'COM_CKFORMS_REQUIRED' ).'</div>'."\n";	
	}
	...
	{ 
		...
		if ($field->typefield != "hidden" && $field->typefield != "button" && $field->typefield != "fieldsep")
		{
			...
			$html = $html.'<p class="ckformsblocplug ckformsbloc_'.$form->id.'" id="ckformsbloc_'.$field->name.'_'.$form->id.'" /><label class="ckCSSlabel ';
			...
			switch ($field->typefield)
			{
				...
				case 'textarea':
					...
					{			
						...
						$html = $html.$editorDesc->display($field->name, $INIThtml, '97%', 200, 0, 0,true);
	...
	if ($form->poweredby == '1') 
	{
		$html = $html.'<div id="ckpoweredby"><a href="http://ckforms.cookex.eu" target="_blank">'.JText::_( 'Produit par CK Forms' ).'</a></div>'."\n";
	}
