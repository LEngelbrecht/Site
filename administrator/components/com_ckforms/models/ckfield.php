 ...
 * @subpackage Components
 */
class CkformsModelCkfield extends JModel
{
	/**
	...
	function store()
	{
		...
		$query = ' SELECT max( ordering ) as maxx FROM #__ckfields WHERE fid = '.$data['fid'];
		$this->_db->setQuery( $query );
		...
		$query = ' SELECT * from #__ckforms c where c.id='.$row->fid.' ';
		$forms = $this->_getList( $query );
		if (count($forms ) > 0)
		{
			$rowTable = $forms[0];
			if ($rowTable->saveresult == 1) {
				//$tn = "j17_ckforms_".$row->fid;	
			   $tn = "#__ckforms_".$row->fid;
				$dba	=& JFactory::getDBO(); 
				$tableFields = $dba->getTableFields($tn,false);
				// on cherche le champ "F".$row->id] dans tablefields alors que la version joomla 1.5 cherchait le champ $row->name
				// qui n'est jamais trouvé car la table ne contient que des champ "F"[id] 
				//La requête était toujours effectuée sans déclencher une erreur en J 1.5
				//ce qui n'est pas le cas en J 2.5
				// La  requête effectuée sous Phpmyadmin [même version pour J1.5 et J 2.5]signale un warning
				//if (!isset($tableFields[$tn][$row->name] )) ancienne version remplée par la ligne quit suit
				
				if (!isset($tableFields[$tn]["F".$row->id] )) 
				{
	...
	function delete()
	{
		...
		if (isset( $fid )) 
		{
			...
			$query = ' SELECT * from #__ckforms c where c.id=' . $fid . ' ';
			$forms = $this->_getList( $query );
			if (count($forms ) > 0)
			{
				$rowTable = $forms[0];
